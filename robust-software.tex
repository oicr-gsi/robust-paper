...
hundreds of tools to perform the same task. In the Bioinformatics
...
\textbf{Provide compile/installation instructions:} If the software needs to be compiled
...
credit your software. READMEs are usually available \emph{before}
...
A program's authors should therefore \textbf{ensure that every released version
...
In all but the simplest of scripts, \textbf{we highly recommend using a build
automation utility to compile code and/or manage your software workflow}. The benefits of
using a build utility like Make, Rake, Maven, Ant or MS Build vastly outweighs the
drawbacks associated with the learning curve. Most importantly, a build system
...
Package managers like apt and yum are available on most *nix systems, and
...
This file can be read by pip package manager, which can check that the
...
In order to be useful, \textbf{this test script must be easy to find and run}. 
...
Equally, \textbf{the test script's output must be easy to interpret}. Screens
