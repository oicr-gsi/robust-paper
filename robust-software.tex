...
usually a graduate student or postdoc, and produces the intended results in their
...
Bioinformatics is full of such efforts, catalogued by repositories such as
...
often they are difficult to install and run~\cite{stajich2002}, and are effectively abandoned
...
the existing training programs and initiatives rarely have the time to cover software engineering
...
That said, not \emph{every} coding effort needs to be engineered to last.
...
help, if they need it. Numerous guidelines exist on how to write good
READMEs\cite{Johnson1997,gnustandards}. We summarize the crucial points here.
...
dependency manager. We address dependencies in more detail in rule
\textcolor{red}{5}.
...
credit your software. READMEs are usualy available \emph{before}
...
The version of your software should be easily available.
Supplying \texttt{-\/-version} or \texttt{-v} on the command line should print
...
continues to be available. A number of mechanisms exist for
...
other project. In addition, every package someone has to install
...
re-use existing software wherever possible and reasonable to avoid
``reinventing the wheel''. We fully support the reuse of software between
projects provided three additional guidelines are adhered to.
...
dependency management system (see Rule \textcolor{red}{RULENO}). 
...
\section{Use a build system and dependency manager.}



If the software depends on a build tool, either because it is written in a
compiled language or to manage a workflow, it should use a standard tool such
as Make rather than home-brewed scripts.
...
package's software be installed in that ``user's'' space (see: Apache Tomcat))
is similarly
...
Lightweight virtualization containers like Docker make this much easier as
well, or simply asking another person to try and build your software.
...
\textbf{Allow the user to configure useful parameters on the command line.}
...
facilitating exploratory analysis and parameter sweeping. 
...
can remove randomness between runs. Allow the user to optionally provide
...
If setting the seed is not possible, make sure the acceptable tolerance is
